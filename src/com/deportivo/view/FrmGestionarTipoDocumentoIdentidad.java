...
import com.deportivo.view.modal.ModalRegistrarTipoDocumentoIdentidad;
...
public class FrmGestionarTipoDocumentoIdentidad extends javax.swing.JInternalFrame {

    public static TipoDocumentoIdentidadController tipoDocumentoC = new TipoDocumentoIdentidadController();
    
    public FrmGestionarTipoDocumentoIdentidad() {
        initComponents();
        listar("");
    }
    ...
    public static void listar(String texto) {
        ...
        if (txtBuscar.getText().length() == 0) {
            lista = tipoDocumentoC.listar();
        } else {
            lista = tipoDocumentoC.buscar(texto);
        }
        Object obj[] = new Object[6];

        for (int i = 0; i < lista.size(); i++) {
            tipoDocumento = (TipoDocumentoIdentidad) lista.get(i);
            obj[0] = tipoDocumento.getTipo_doc_identidad_id();
            obj[1] = tipoDocumento.getDescripcion_larga();
            obj[2] = tipoDocumento.getAbreviatura();
    ...
    private void initComponents() {
        ...
        setIconifiable(true);
        setMaximizable(true);
        setTitle("GESTIONAR TIPO DE DOCUMENTO DE IDENTIDAD");
    ...
    private void btnAddActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAddActionPerformed
        ...
        ModalRegistrarTipoDocumentoIdentidad frm = new ModalRegistrarTipoDocumentoIdentidad();
                ...
                switch (boton.getName()) {
                    case "btnEliminar" -> {
                        if (filas == 0) {//si no elije ninguna fila
                            Alerta alerta = new Alerta("Alerta", "Debe seleccionar un tipo de documento");
                        } else {
                            String valor = String.valueOf(tblListado.getValueAt(fila, 1));

                            int opcion = JOptionPane.showConfirmDialog(null, "¿Está seguro de eliminar el Tipo de documento " + valor + "?", "Confirmar", 2);
                            if (opcion == 0) {

                                try {
                                    tipoDocumentoC.eliminar(id);
                                    AlertaBien alertaBien = new AlertaBien("Mensaje", "Tipo de documento eliminado correctamente!");
                                    listar("");
                                } catch (Exception ex) {
                    ...
                    case "btnModificar" -> {
                        if (filas == 0) {//si no elije ninguna fila
                            ...
                            ModalRegistrarTipoDocumentoIdentidad.idTipoDocumento = id;
                            FrmMenuPrincipal.centrarVentana(new ModalRegistrarTipoDocumentoIdentidad());
                            ModalRegistrarTipoDocumentoIdentidad.btnGrabar.setText("Modificar");
                    ...
                    case "btnVer" -> {
                        if (filas == 0) {
                            ...
                            ModalRegistrarTipoDocumentoIdentidad.vista = true;
                            ModalRegistrarTipoDocumentoIdentidad.idTipoDocumento = id;
                            FrmMenuPrincipal.centrarVentana(new ModalRegistrarTipoDocumentoIdentidad());
                        }
                    }
