...
import com.deportivo.view.modal.ModalRegistrarEvento;
...
public class FrmGestionarEvento extends javax.swing.JInternalFrame {

    public static EventoController eventoC = new EventoController();
    
    public FrmGestionarEvento() {
        initComponents();
        listar("");
    }
    ...
    public static void listar(String texto) {
        ...
        if (txtBuscar.getText().length() == 0) {
            lista = eventoC.listar();
        } else {
            lista = eventoC.buscar(texto);
        }
        Object obj[] = new Object[5];

        for (int i = 0; i < lista.size(); i++) {
            evento = (Evento) lista.get(i);
            obj[0] = evento.getEventoId();
    ...
    private void btnAddActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAddActionPerformed
        ...
        ModalRegistrarEvento frm = new ModalRegistrarEvento();
                ...
                switch (boton.getName()) {
                    case "btnEliminar" -> {
                        if (filas == 0) {//si no elije ninguna fila
                            Alerta alerta = new Alerta("Alerta", "Debe seleccionar un evento");
                        } else {
                            String valor = String.valueOf(tblListado.getValueAt(fila, 1));

                            int opcion = JOptionPane.showConfirmDialog(null, "¿Está seguro de eliminar al evento " + valor + "?", "Confirmar", 2);
                            if (opcion == 0) {

                                try {
                                    eventoC.eliminar(id);
                                    AlertaBien alertaBien = new AlertaBien("Mensaje", "Evento eliminado correctamente!");
                                    listar("");
                                } catch (Exception ex) {
                    ...
                    case "btnModificar" -> {
                        if (filas == 0) {//si no elije ninguna fila
                            ...
                            ModalRegistrarEvento.idEvento = id;
                            FrmMenuPrincipal.centrarVentana(new ModalRegistrarEvento());
                            ModalRegistrarEvento.btnGrabar.setText("Modificar");
                    ...
                    case "btnVer" -> {
                        if (filas == 0) {
                            ...
                            ModalRegistrarEvento.vista = true;
                            ModalRegistrarEvento.idEvento = id;
                            FrmMenuPrincipal.centrarVentana(new ModalRegistrarEvento());
                        }
                    }
