...
import com.deportivo.view.modal.ModalRegistrarFixture;
...
public class FrmGestionarFixture extends javax.swing.JInternalFrame {

    public static FixtureController fixtureC = new FixtureController();

    public FrmGestionarFixture() {
        initComponents();
        listar("");
    }
    ...
    public static void listar(String texto) {
        ...
        if (txtBuscar.getText().length() == 0) {
            lista = fixtureC.listar();
        } else {
            lista = fixtureC.buscar(texto);
        }
        Object obj[] = new Object[6];

        for (int i = 0; i < lista.size(); i++) {
            fixture = (Fixture) lista.get(i);
            obj[0] = fixture.getFixture_id();
            obj[1] = fixture.getNombre();
    ...
    private void btnAddActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAddActionPerformed
        ...
        ModalRegistrarFixture frm = new ModalRegistrarFixture();
                ...
                switch (boton.getName()) {
                    case "btnEliminar" -> {
                        if (filas == 0) {//si no elije ninguna fila
                            Alerta alerta = new Alerta("Alerta", "Debe seleccionar un fixture");
                        } else {
                            String valor = String.valueOf(tblListado.getValueAt(fila, 1));

                            int opcion = JOptionPane.showConfirmDialog(null, "¿Está seguro de eliminar al fixture " + valor + "?", "Confirmar", 2);
                            if (opcion == 0) {

                                try {
                                    fixtureC.eliminar(id);
                                    AlertaBien alertaBien = new AlertaBien("Mensaje", "Fixture eliminado correctamente!");
                                    listar("");
                                } catch (Exception ex) {
                    ...
                    case "btnModificar" -> {
                        if (filas == 0) {//si no elije ninguna fila
                            ...
                            ModalRegistrarFixture.idFixture = id;
                            FrmMenuPrincipal.centrarVentana(new ModalRegistrarFixture());
                            ModalRegistrarFixture.btnGrabar.setText("Modificar");
                    ...
                    case "btnVer" -> {
                        if (filas == 0) {
                            ...
                            ModalRegistrarFixture.vista = true;
                            ModalRegistrarFixture.idFixture = id;
                            FrmMenuPrincipal.centrarVentana(new ModalRegistrarFixture());
                        }
                    }
