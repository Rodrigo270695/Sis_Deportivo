...
import com.deportivo.view.modal.ModalRegistrarAgente;
...
public class FrmGestionarAgente extends javax.swing.JInternalFrame {

    public static AgenteController agenteC = new AgenteController();
    ...
    public static void listar(String texto) {
        ...
        String columas[] = {"#", "NOMBRE", "DIRECCIÓN", "CORREO", "", "", ""};
        ...
        if (txtBuscar.getText().length() == 0) {
            lista = agenteC.listar();
        } else {
            lista = agenteC.buscar(texto);
        }
        Object obj[] = new Object[7];

        for (int i = 0; i < lista.size(); i++) {
            agente = (Agente) lista.get(i);
            obj[0] = agente.getAgenteId();
            obj[1] = agente.getNombreCompleto();
            obj[2] = agente.getDireccion();
            obj[3] = agente.getCorreo();
        ...
        tblListado.getColumnModel().getColumn(2).setPreferredWidth(227);
        tblListado.getColumnModel().getColumn(3).setPreferredWidth(227);
    ...
    private void initComponents() {
        ...
        btnAdd.setFont(new java.awt.Font("Segoe UI Black", 1, 30)); // NOI18N
    ...
    private void btnAddActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAddActionPerformed
        ...
        ModalRegistrarAgente frm = new ModalRegistrarAgente();
                ...
                switch (boton.getName()) {
                    case "btnEliminar" -> {
                        if (filas == 0) {//si no elije ninguna fila
                            Alerta alerta = new Alerta("Alerta", "Debe seleccionar un agente");
                        } else {
                            String valor = String.valueOf(tblListado.getValueAt(fila, 1));

                            int opcion = JOptionPane.showConfirmDialog(null, "¿Está seguro de eliminar al agente " + valor + "?", "Confirmar", 2);
                            if (opcion == 0) {

                                try {
                                    agenteC.eliminar(id);
                                    AlertaBien alertaBien = new AlertaBien("Mensaje", "Agente eliminado correctamente!");
                                    listar("");
                                } catch (Exception ex) {
                    ...
                    case "btnModificar" -> {
                        if (filas == 0) {//si no elije ninguna fila
                            ...
                            ModalRegistrarAgente.idAgente = id;
                            FrmMenuPrincipal.centrarVentana(new ModalRegistrarAgente());
                            ModalRegistrarAgente.btnGrabar.setText("Modificar");
                    ...
                    case "btnVer" -> {
                        if (filas == 0) {
                            ...
                            ModalRegistrarAgente.vista = true;
                            ModalRegistrarAgente.idAgente = id;
                            FrmMenuPrincipal.centrarVentana(new ModalRegistrarAgente());
                        }
                    }
