...
import com.deportivo.view.modal.ModalRegistrarTipoContrato;
...
public class FrmGestionarTipoContrato extends javax.swing.JInternalFrame {

    public static TipoContratoController tipoContratoC = new TipoContratoController();
    
    public FrmGestionarTipoContrato() {
        initComponents();
        listar("");
    }
    ...
    public static void listar(String texto) {
        ...
        if (txtBuscar.getText().length() == 0) {
            lista = tipoContratoC.listar();
        } else {
            lista = tipoContratoC.buscar(texto);
        }
        Object obj[] = new Object[5];

        for (int i = 0; i < lista.size(); i++) {
            tipoContrato = (TipoContrato) lista.get(i);
            obj[0] = tipoContrato.getTipoContratoId();
            obj[1] = tipoContrato.getDescripcion();
    ...
    private void initComponents() {
        ...
        setIconifiable(true);
        setMaximizable(true);
        setTitle("GESTIONAR TIPO DE CONTRATO");
    ...
    private void btnAddActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAddActionPerformed
        ...
        ModalRegistrarTipoContrato frm = new ModalRegistrarTipoContrato();
                ...
                switch (boton.getName()) {
                    case "btnEliminar" -> {
                        if (filas == 0) {//si no elije ninguna fila
                            Alerta alerta = new Alerta("Alerta", "Debe seleccionar un tipoContrato");
                        } else {
                            String valor = String.valueOf(tblListado.getValueAt(fila, 1));

                            int opcion = JOptionPane.showConfirmDialog(null, "¿Está seguro de eliminar al tipoContrato " + valor + "?", "Confirmar", 2);
                            if (opcion == 0) {

                                try {
                                    tipoContratoC.eliminar(id);
                    ...
                    case "btnModificar" -> {
                        if (filas == 0) {//si no elije ninguna fila
                            ...
                            ModalRegistrarTipoContrato.idTipoContrato = id;
                            FrmMenuPrincipal.centrarVentana(new ModalRegistrarTipoContrato());
                            ModalRegistrarTipoContrato.btnGrabar.setText("Modificar");
                    ...
                    case "btnVer" -> {
                        if (filas == 0) {
                            ...
                            ModalRegistrarTipoContrato.vista = true;
                            ModalRegistrarTipoContrato.idTipoContrato = id;
                            FrmMenuPrincipal.centrarVentana(new ModalRegistrarTipoContrato());
                        }
                    }
