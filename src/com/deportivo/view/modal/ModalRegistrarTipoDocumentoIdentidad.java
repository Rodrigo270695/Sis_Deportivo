...
import com.deportivo.controller.TipoDocumentoIdentidadController;
import com.deportivo.model.TipoDocumentoIdentidad;
import com.deportivo.view.FrmGestionarTipoDocumentoIdentidad;
...
public final class ModalRegistrarTipoDocumentoIdentidad extends javax.swing.JInternalFrame {

    TipoDocumentoIdentidadController tipoDocumentoC = new TipoDocumentoIdentidadController();
    public static boolean activarTipoDocumentoIdentidad = false;
    public static int idTipoDocumento = 0;
    public static boolean vista = false;

    public ModalRegistrarTipoDocumentoIdentidad() {
        initComponents();
        acciones();
    }
    ...
    void acciones() {
        ...
        if (idTipoDocumento > 0) {
            ...
            TipoDocumentoIdentidad tipoDocumento = (TipoDocumentoIdentidad) tipoDocumentoC.obtenerdato(idTipoDocumento);
            txtDescripcion.setText(tipoDocumento.getDescripcion_larga());
            txtAbreviatura.setText(tipoDocumento.getAbreviatura());
    ...
    void grabar() {
        ...
        if (btnGrabar.getText().equalsIgnoreCase("Grabar")) {
            ...
            tipoDocumento.setDescripcion_larga(txtDescripcion.getText().toUpperCase());
            tipoDocumento.setAbreviatura(txtAbreviatura.getText().toUpperCase());

            try {
                tipoDocumentoC.registrar(tipoDocumento);
                AlertaBien bien = new AlertaBien("Mensaje", "Se registró correctamente el tipo de documento de identidad");

                if (activarTipoDocumentoIdentidad == false) {
                    FrmGestionarTipoDocumentoIdentidad.listar("");
                } else {
//                    ModalRegistrarDetalleProfesional.cargarTipoDocumentoIdentidades();
                }
                dispose();
            } catch (Exception e) {
            ...
            tipoDocumento.setDescripcion_larga(txtDescripcion.getText().toUpperCase());
            tipoDocumento.setAbreviatura(txtAbreviatura.getText().toUpperCase());
            tipoDocumento.setTipo_doc_identidad_id(idTipoDocumento);

            try {
                tipoDocumentoC.modificar(tipoDocumento);
                AlertaBien bien = new AlertaBien("Mensaje", "Se modificó correctamente el tipo de documento de identidad");
                FrmGestionarTipoDocumentoIdentidad.listar("");
                dispose();
            } catch (Exception e) {
                AlertaError err = new AlertaError("Error", e.getMessage());
    ...
    private void initComponents() {
        ...
        setClosable(true);
        setIconifiable(true);
        setTitle("REGISTRAR TIPO DOCUMENTO DE IDENTIDAD");
