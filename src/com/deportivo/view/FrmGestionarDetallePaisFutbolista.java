...
import com.deportivo.controller.DetallePaisFutbolistaController;
...
import com.deportivo.view.modal.ModalRegistrarDetallePaisFutbolista;
import com.deportivo.vista.modal.alerts.*;
import java.awt.*;
import javax.swing.*;
import javax.swing.table.DefaultTableModel;

public class FrmGestionarDetallePaisFutbolista extends javax.swing.JInternalFrame {

    public static DetallePaisFutbolistaController detalleC = new DetallePaisFutbolistaController();
    PaisController tipoC = new PaisController();
    public static int idFutbolista2;

    public FrmGestionarDetallePaisFutbolista() {
        initComponents();
        listar("");
    }
    ...
    public static void listar(String texto) {
        ...
        DetallePaisFutbolista detallePro = null;
        Pais tipoP;

        detallePro = detalleC.listar(idFutbolista2);

        Object obj[] = new Object[2];

        for (int i = 0; i < detallePro.getPais().size(); i++) {
            tipoP = detallePro.getPais().get(i);
    ...
    private void initComponents() {
        ...
        setClosable(true);
        setIconifiable(true);
        setTitle("GESTIONAR DETALLE PAÍS FUTBOLISTA");
    ...
    private void btnAddActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAddActionPerformed
        ...
        ModalRegistrarDetallePaisFutbolista.idFutbolista2 = idFutbolista2;
        ModalRegistrarDetallePaisFutbolista frm = new ModalRegistrarDetallePaisFutbolista();
                ...
                switch (boton.getName()) {
                    case "btnEliminar" -> {
                        if (filas == 0) {//si no elije ninguna fila
                            ...
                            int opcion = JOptionPane.showConfirmDialog(null, "¿Está seguro de eliminar el Detalle País Futbolista " + valor + "?", "Confirmar", 2);
                            if (opcion == 0) {

                                Pais tipoP = (Pais) tipoC.obtenerdato(valor);

                                try {
                                    detalleC.eliminarDetalle(idFutbolista2, tipoP.getPaisId());
                                    AlertaBien alertaBien = new AlertaBien("Mensaje", "Detalle País Futbolista eliminado correctamente!");
                                    listar("");
                                } catch (Exception ex) {
