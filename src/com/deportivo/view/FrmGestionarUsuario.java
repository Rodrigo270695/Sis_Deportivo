...
import com.deportivo.view.modal.ModalRegistrarUsuario;
...
public class FrmGestionarUsuario extends javax.swing.JInternalFrame {
    ...
    public static void listar(String texto) {
        ...
        String columas[] = {"#", "Rol", "Nombre", "Documento", "","",""};
        ...
        if (txtBuscar.getText().length() == 0) {
            ...
            lista = usuarioC.buscar(texto);
        }
        Object obj[] = new Object[7];

        for (int i = 0; i < lista.size(); i++) {
            usuario = (Usuario) lista.get(i);
            obj[0] = usuario.getUsuarioId();
            obj[1] = usuario.getRolId().getNombre();
            obj[2] = usuario.getNombre();
            obj[3] = usuario.getDocumento();
        ...
        tblListado.getColumnModel().getColumn(2).setPreferredWidth(175);
    ...
    private void btnAddActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAddActionPerformed
        ...
        ModalRegistrarUsuario frm = new ModalRegistrarUsuario();
                ...
                switch (boton.getName()) {
                    case "btnEliminar" -> {
                        if (filas == 0) {//si no elije ninguna fila
                            Alerta alerta = new Alerta("Alerta", "Debe seleccionar un usuario");
                        } else {
                            String valor = String.valueOf(tblListado.getValueAt(fila, 2));

                            int opcion = JOptionPane.showConfirmDialog(null, "¿Está seguro de eliminar al usuario " + valor + "?", "Confirmar", 2);
                            if (opcion == 0) {

                                try {
                                    usuarioC.eliminar(id);
                                    AlertaBien alertaBien = new AlertaBien("Mensaje", "Usuario eliminado correctamente!");
                                    listar("");
                                } catch (Exception ex) {
                    ...
                    case "btnModificar" -> {
                        if (filas == 0) {//si no elije ninguna fila
                            ...
                            ModalRegistrarUsuario.idUsuario = id;
                            FrmMenuPrincipal.centrarVentana(new ModalRegistrarUsuario());
                            ModalRegistrarUsuario.btnGrabar.setText("Modificar");
                    ...
                    case "btnVer" -> {
                        if (filas == 0) {
                            ...
                            ModalRegistrarUsuario.vista = true;
                            ModalRegistrarUsuario.idUsuario = id;
                            FrmMenuPrincipal.centrarVentana(new ModalRegistrarUsuario());
                        }
                    }
